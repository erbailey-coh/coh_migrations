...
| `edw_sem.patient_billing.sem_billing_transactions` | Unified hospital & professional charge fact | Combination of: <br>`ref('ae_enterprise_dbt', 'unified_billing_transactions')` for charge metrics and descriptors.<br>`ref('ae_enterprise_dbt', 'professional_transactions')` (or staging) for POS type code & modifiers needed for telehealth logic.<br>`ref('ae_enterprise_dbt', 'hospital_transactions')` for `revenue_code` when not exposed in unified view. | Need to ensure resulting dataset carries `billing_module`, `revenue_code`, `is_em_charge`, modifiers array, POS type, and `transaction_type`. |
